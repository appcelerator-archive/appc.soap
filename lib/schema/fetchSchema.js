...
exports.fetchSchema = function (next) {
	...
	function discoverModels(key, context, child) {
		if (_.isFunction(child)) {
			...
			method.verbs = [];
			if (messages[methodName + 'HttpGetIn']) { method.verbs.push('GET'); }
			if (messages[methodName + 'HttpPostIn']) { method.verbs.push('POST'); }
			if (messages[methodName + 'HttpPutIn']) { method.verbs.push('PUT'); }
			if (messages[methodName + 'HttpDeleteIn']) { method.verbs.push('DELETE'); }

			if (method.verbs.length) {
